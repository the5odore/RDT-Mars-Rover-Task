...
class SetCoordinates
{
    static void Main()
    {
        for (int x = 0; x < 1;) //Infinite loop, so the program keeps running until manually stopped
        {
            ...
            if (plateauCoordinates.Length != 2) //Checks if the input has exactly 2 elements (x and y)
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < roverCount; i++) //Loops through the number of rovers specified by the user, incrementing it by one, if the requirements meet
                    {
                        ...
                        if (rovercoordinates.Length != 3) //Checks if the input has exactly 3 elements (x, y, direction)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        x = 1; //Resets the loop to allow for new plateau and rover inputs
                                    }
...
class Rover1 //create a class called Rover
{
    ...
    bool isvalid = true; //boolean variable, set to true. Used to check if the Rover is in a valid position
    ...
    public void ExecuteCommands(string commands) // public method, used get multiple commands in one row
    {
        foreach (char command in commands) // The foreach loop will break down the string into individual characters, where it will go through each character one by one
        {
            if (!isvalid)
            {
                break; //If the rover is not in a valid position, it breaks out of the loop
            }
            else
            {
    ...
    private void MoveForward() //private method, set to change the direction of the Rover foward
    {
        switch (direction)
        {
            case 'N': //if the direction is North, increase y by 1. Moving it North (up). 
                ...
                {
                    Console.WriteLine("Rover is out of bounds!");
                    isvalid = false;
                    break;
                }
            ...
            case 'W':
                if (x - 1 > 0) //if the direction is West, decrease x by 1. Moving it West (left).
                {
                    x -= 1;
                    break;
                }
    ...
    {
        if (!isvalid) //Checks if the rover is in a valid position
        {
            return "No Valid Input"; //Outputs a message if the rover is out of bounds
        }
        else
        {
